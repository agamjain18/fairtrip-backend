...
from database import get_db, Photo, Poll, PollOption, PollVote, BucketListItem, Accommodation, Flight, Notification
from schemas import (
...
def vote_on_poll(poll_id: int, option_id: int, user_id: int, db: Session = Depends(get_db)):
    ...
    existing_vote = db.query(PollVote).join(PollOption).filter(
